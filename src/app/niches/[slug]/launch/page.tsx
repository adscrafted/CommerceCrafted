...
export default function NicheLaunchPage({ params }: LaunchPageProps) {
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      try {
        ...
        const response = await fetch(`/api/niches/${resolvedParams.slug}/launch`)
  ...
  const transformedData = {
    ...
    scores: {
      launch: 85 // Default score
    },
    launchStrategyData: launchData?.launchStrategy || {},
    hasData: launchData?.hasData || false
  }
            ...
            <div>
              <div className="flex items-center space-x-3 mb-3">
                ...
                <div>
                  ...
                  <p className="text-base text-gray-600">Timeline, marketing plan & success metrics</p>
                </div>
              </div>
            </div>
            ...
            <Card className="border-2 border-indigo-200">
              <CardContent className="p-4">
                <div className="text-center w-32 h-20 flex flex-col items-center justify-center">
                  <div className={`text-3xl font-bold ${getScoreColor(transformedData.scores.launch)}`}>
                    {transformedData.scores.launch}
                  </div>
                  <div className="text-xs text-gray-600 mt-1">{getScoreLabel(transformedData.scores.launch)}</div>
                  <Progress value={transformedData.scores.launch} className="h-2 mt-2 w-full" />
                </div>
              </CardContent>
            </Card>
          </div>
