...
interface DemandAnalysisProps {
  data: {
    demandData: {
      ...
      _nicheProducts?: Array<any>
      _priceHistory?: Array<any>
    }
  }
  nicheId?: string | null
  nicheData?: any
...
export default function DemandAnalysis({ data, nicheId, nicheData }: DemandAnalysisProps) {
  ...
  const [demandAnalysisData, setDemandAnalysisData] = useState<any>(null)
  ...
  const toggleCompetitor = (asin: string) => {
    setSelectedCompetitors(prev => {
      // If trying to deselect the last selected competitor, don't allow it
  ...
  useEffect(() => {
    const fetchDemandAnalysis = async () => {
      ...
      try {
        const response = await fetch(`/api/niches/${nicheId}/demand-analysis`)
        const result = await response.json()
        
        if (response.ok) {
          setDemandAnalysisData(result)
          console.log('Fetched demand analysis data:', result)
        }
      } catch (error) {
  ...
  return (
    <div className="space-y-6">
      ...
      {activeTab === 'market' && (
        <div className="space-y-6">
          ...
          <Card>
            ...
            <CardContent>
              {demandAnalysisLoading || aiLoading ? (
              ...
              ) : demandAnalysisData?.hasData && demandAnalysisData?.marketInsights ? (
                <div className="space-y-6">
                  {/* Display market insights from niches_demand_analysis table */}
                  {demandAnalysisData.marketInsights.marketPhase && (
                    <div className="p-4 bg-gradient-to-r from-purple-50 to-pink-50 rounded-lg">
                      ...
                      <div className="grid grid-cols-2 gap-4">
                        <div>
                          ...
                          <p className="text-sm font-semibold text-gray-900 capitalize">{demandAnalysisData.marketInsights.marketPhase}</p>
                        </div>
                        <div>
                          <span className="text-xs text-gray-600">Market Maturity</span>
                          <p className="text-sm font-semibold text-gray-900 capitalize">{demandAnalysisData.marketInsights.marketMaturity || 'Analyzing...'}</p>
                        </div>
                      </div>
                      {demandAnalysisData.marketInsights.growthIndicators && (
                        <div className="mt-3">
                          <span className="text-xs text-gray-600">Growth Indicators</span>
                          <ul className="mt-1 space-y-1">
                            {demandAnalysisData.marketInsights.growthIndicators.map((indicator: string, idx: number) => (
                  ...
                  {/* Industry Insights from demand analysis */}
                  {demandAnalysisData.marketInsights.industryInsights && (
                    <div>
                      <h4 className="font-medium text-gray-900 mb-3">Industry Trends</h4>
                      <div className="space-y-3">
                        {demandAnalysisData.marketInsights.industryInsights.map((insight: any, idx: number) => (
                  ...
                  {/* Demand Patterns from demand analysis */}
                  {demandAnalysisData.marketInsights.demandPatterns && (
                    <div className="p-4 bg-blue-50 rounded-lg">
                      <h4 className="font-medium text-gray-900 mb-3">Demand Patterns</h4>
                      <div className="space-y-2">
                        {Object.entries(demandAnalysisData.marketInsights.demandPatterns).map(([key, value]: [string, any]) => (
      ...
      {activeTab === 'seasonality' && (
        <div className="space-y-6">
          {/* AI Seasonality Insights from demand analysis */}
          {demandAnalysisData?.hasData && demandAnalysisData?.seasonalityInsights && (
            <Card>
              <CardHeader>
                <CardTitle className="flex items-center space-x-2">
                  <Sparkles className="h-5 w-5 text-purple-600" />
                  <span>AI Seasonality Analysis</span>
                </CardTitle>
                <CardDescription>
                  AI-generated insights on seasonal patterns and opportunities
                </CardDescription>
              </CardHeader>
              <CardContent>
                <div className="space-y-4">
                  {demandAnalysisData.seasonalityInsights.peakSeasons && Array.isArray(demandAnalysisData.seasonalityInsights.peakSeasons) && (
                    <div className="p-4 bg-orange-50 rounded-lg">
                      <h4 className="font-medium text-gray-900 mb-3">Peak Seasons</h4>
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                        {demandAnalysisData.seasonalityInsights.peakSeasons.map((season: string, idx: number) => (
                          <div key={idx} className="flex items-center space-x-2 p-3 bg-white rounded-lg">
                            <Calendar className="h-4 w-4 text-orange-500" />
                            <span className="text-sm font-medium text-gray-800">{season}</span>
                          </div>
                        ))}
                      </div>
                    </div>
                  ...
                  {demandAnalysisData.seasonalityInsights.demandPatterns && Array.isArray(demandAnalysisData.seasonalityInsights.demandPatterns) && (
                    <div>
                      <h4 className="font-medium text-gray-900 mb-3">Seasonal Demand Patterns</h4>
                      <div className="space-y-2">
                        {demandAnalysisData.seasonalityInsights.demandPatterns.map((pattern: string, idx: number) => (
                          <div key={idx} className="flex items-start space-x-2">
                  ...
                  {demandAnalysisData.seasonalityInsights.yearRoundViability && (
                    <div className="p-4 bg-blue-50 rounded-lg">
                      <h4 className="font-medium text-gray-900 mb-2">Year-Round Viability</h4>
                      <p className="text-sm text-gray-700">{demandAnalysisData.seasonalityInsights.yearRoundViability}</p>
                    </div>
                  )}
                  
                  {(!demandAnalysisData.seasonalityInsights.peakSeasons && 
                    !demandAnalysisData.seasonalityInsights.demandPatterns && 
                    !demandAnalysisData.seasonalityInsights.yearRoundViability) && (
                    <div className="p-4 bg-gray-50 rounded-lg text-center">
                      <p className="text-sm text-gray-600">Seasonality analysis will be generated during niche processing.</p>
                    </div>
                  )}
                </div>
              </CardContent>
          ...
          <Card>
            ...
            <CardContent>
              <div className="space-y-6">
                    ...
                    <ResponsiveContainer width="100%" height="100%">
                      <LineChart 
                        data={(() => {
                          ...
                          // Use weekly data points
                          const dataPoints = 52
                          const daysBetween = 7
                          
                          const rawData = Array.from({length: dataPoints}, (_, i) => {
                            const date = new Date()
                            date.setDate(date.getDate() - (dataPoints - i) * daysBetween)
                            ...
                            if (viewMode === 'individual' && selectedCompetitors.length > 0) {
                              ...
                              dataPoint.rank = Math.round(Math.max(1000, avgRank))
                            }
                ...
                {/* Competitor Cards */}
                <div className={`relative z-0 ${data.demandData._nicheProducts?.length > 12 ? 'max-h-[480px] overflow-y-auto pr-2' : ''}`}>
      ...
      {activeTab === 'social' && (
        <div className="space-y-6">
          {/* AI Social Signals from demand analysis */}
          {demandAnalysisData?.hasData && demandAnalysisData?.socialSignals && (
            <Card>
              <CardHeader>
                <CardTitle className="flex items-center space-x-2">
                  <Sparkles className="h-5 w-5 text-purple-600" />
                ...
                <CardDescription>
                  AI-generated insights on social media trends and viral potential
                </CardDescription>
              </CardHeader>
              <CardContent>
                {demandAnalysisData.socialSignals.viralPotential && (
                  <div className="space-y-4">
                    <div className="p-4 bg-gradient-to-r from-pink-50 to-purple-50 rounded-lg">
                      <h4 className="font-medium text-gray-900 mb-2">Viral Potential Score</h4>
                      <div className="flex items-center space-x-4">
                        <div className="text-3xl font-bold text-purple-600">
                          {demandAnalysisData.socialSignals.viralPotential.score}/100
                        </div>
                        <Badge variant="secondary" className="text-sm">
                          {demandAnalysisData.socialSignals.viralPotential.level}
                        </Badge>
                      </div>
                      {demandAnalysisData.socialSignals.viralPotential.factors && (
                        <div className="mt-3">
                          <p className="text-xs text-gray-600">Key Factors:</p>
                          <p className="text-sm text-gray-700">{demandAnalysisData.socialSignals.viralPotential.factors.join(', ')}</p>
                        </div>
                      )}
                    </div>
                    
                    {demandAnalysisData.socialSignals.platformInsights && (
                      <div>
                        <h4 className="font-medium text-gray-900 mb-3">Platform-Specific Insights</h4>
                        <div className="space-y-2">
                          {Object.entries(demandAnalysisData.socialSignals.platformInsights).map(([platform, insights]: [string, any]) => (
                            <div key={platform} className="p-3 border rounded-lg">
                              <div className="flex items-center justify-between mb-2">
                                <h5 className="text-sm font-medium text-gray-900 capitalize">{platform}</h5>
                                {insights.trending && <Badge variant="destructive">Trending</Badge>}
                              </div>
                              <p className="text-xs text-gray-600">{insights.description}</p>
                              {insights.contentTypes && (
                                <div className="mt-2">
                                  <span className="text-xs text-gray-500">Popular content: </span>
                                  <span className="text-xs text-gray-700">{insights.contentTypes.join(', ')}</span>
                                </div>
                              )}
                            </div>
                          ))}
                    ...
                    {demandAnalysisData.socialSignals.influencerOpportunities && (
                      <div className="p-4 bg-yellow-50 rounded-lg">
                        <h4 className="font-medium text-gray-900 mb-2">Influencer Marketing Opportunities</h4>
                        <p className="text-sm text-gray-700">{demandAnalysisData.socialSignals.influencerOpportunities}</p>
                      </div>
                    )}
                  </div>
                )}
      ...
      {activeTab === 'pricing' && (
        <div className="space-y-6">
          {/* AI Pricing Insights from demand analysis */}
          {demandAnalysisData?.hasData && demandAnalysisData?.pricingTrends && (
            <Card>
              <CardHeader>
                <CardTitle className="flex items-center space-x-2">
                  <Sparkles className="h-5 w-5 text-purple-600" />
                ...
                <CardDescription>
                  AI-generated insights on pricing strategies and trends
                </CardDescription>
              </CardHeader>
              <CardContent>
                {demandAnalysisData.pricingTrends.priceOptimization && (
                  <div className="space-y-4">
                    {demandAnalysisData.pricingTrends.optimalPriceRange && (
                      <div className="p-4 bg-green-50 rounded-lg">
                        <h4 className="font-medium text-gray-900 mb-2">Optimal Price Range</h4>
                        <div className="flex items-center space-x-4">
                          <div className="text-2xl font-bold text-green-600">
                            ${demandAnalysisData.pricingTrends.optimalPriceRange.min} - ${demandAnalysisData.pricingTrends.optimalPriceRange.max}
                          </div>
                          {demandAnalysisData.pricingTrends.optimalPriceRange.sweetSpot && (
                            <Badge variant="secondary">Sweet Spot: ${demandAnalysisData.pricingTrends.optimalPriceRange.sweetSpot}</Badge>
                          )}
                        </div>
                      </div>
                    )}
                    
                    {demandAnalysisData.pricingTrends.priceStrategies && (
                      <div>
                        <h4 className="font-medium text-gray-900 mb-3">Pricing Strategies</h4>
                        <div className="space-y-2">
                          {demandAnalysisData.pricingTrends.priceStrategies.map((strategy: any, idx: number) => (
                            <div key={idx} className="p-3 border rounded-lg">
                              <div className="flex items-start justify-between">
                                ...
                                {strategy.effectiveness && (
                                  <Badge variant="outline">{strategy.effectiveness}</Badge>
                                )}
                              </div>
                            </div>
                          ))}
                    ...
                    {demandAnalysisData.pricingTrends.competitorInsights && (
                      <div className="p-4 bg-blue-50 rounded-lg">
                        <h4 className="font-medium text-gray-900 mb-2">Competitor Pricing Insights</h4>
                        <p className="text-sm text-gray-700">{demandAnalysisData.pricingTrends.competitorInsights}</p>
                      </div>
                    )}
                  </div>
                )}
                          ...
                          const rawData = Array.from({length: dataPoints}, (_, i) => {
                            ...
                            if (viewMode === 'individual' && selectedCompetitors.length > 0) {
                              selectedCompetitors.forEach(asin => {
                                ...
                                const seasonalFactor = Math.sin((i / dataPoints) * Math.PI * 2) * 0.1
                                const randomFactor = (Math.random() - 0.5) * 0.05
                                const trendFactor = (i / dataPoints) * 0.02
                              ...
                              dataPoint.price = Number(avgPrice.toFixed(2))
                            }
            ...
            <CardContent>
              {data.keywordHierarchy ? (
                <KeywordNetworkVisualization
                  ...
                  productImageUrl={(() => {
                    // Get the first product image from nicheData or data
                    if (nicheData?.products && nicheData.products.length > 0) {
                      const firstProduct = nicheData.products[0]
                      if (firstProduct.image_urls) {
                        try {
                          const urls = typeof firstProduct.image_urls === 'string' 
                            ? JSON.parse(firstProduct.image_urls) 
                            : firstProduct.image_urls
                          const imageUrl = Array.isArray(urls) ? urls[0] : urls
                          return imageUrl && !imageUrl.startsWith('http') 
                            ? `https://m.media-amazon.com/images/I/${imageUrl}`
                            : imageUrl
                        } catch {
                          const urls = firstProduct.image_urls.split(',').map((url: string) => url.trim())
                    ...
                    // Fallback to default image if no product image available
